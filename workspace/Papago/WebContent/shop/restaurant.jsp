...
<body>
			...
			<c:if test="${not empty select}">
				<div class="container">
					...
					<table class="table table-hover">
						<c:forEach var="bean" items="${select}">
							<tr>
								<td><strong><font size="4">${bean.name}</font></strong> <figure
										style="float: right;"> <c:if
										test="${not empty bean.shopPic}">
										<img src="${bean.shopPic}" width="100" border="0">
									</c:if> </figure><br> 類型：${bean.shopType.shopType}
									店家狀態：${bean.shopStatusType.shopStatus}<br> 地址：<span
									id="address">${bean.shopAddress}</span><br>
									電子信箱：${bean.email}<br> <c:if test="${not empty bean.web}">
 									官方網站：<a href="${bean.web}" target="_blank">有</a>
										<br>
									</c:if> 電話：${bean.phone}<br> 價格：${bean.price}
									價位區間：${bean.priceType.priceType}<br> <c:if
										test="${not empty bean.note}">
										<blockquote>
											店家備註<br> ${bean.note}<br>
										</blockquote>
									</c:if></td>
							</tr>

							<tr>
								<td><c:if test="${not empty bean.contactName}">聯絡人：${bean.contactName}　聯絡人電話：${bean.contactPhone}<br>
									</c:if>
									<div align="right">
										最後更新日期：${bean.shopDate}<br> <a href='<c:url value="" />'
											target="_blank" class="smalla">店家資料錯誤回報</a>
									</div></td>
							</tr>
						</c:forEach>
					</table>
	...
	<!--  留言部分-->
	<h1>哈哈哈${user.memType.memtypeId}</h1>
	<c:set value="${user.memType.memtypeId}" var="prior"></c:set>
	<h2>對這個店家留個言吧，目前尚有 ${fn:length(msg)} 篇留言。</h2>
	<c:if test="${not empty msg}">
		<ul class="list-group">
			<c:forEach var="bean" items="${msg}">
				<li class="list-group-item"><a href="#"> <img src="#"
						class="avatar">
				</a>
					<p class="name">
						<a href="">${bean.papaMem.name}</a> <span class="date"><fmt:formatDate
								value="${bean.msgDate}" pattern="yyyy/MM/dd EEEE" /></span>
					</p>
	...
	<form action="<c:url value="/shop/papashop.controller" />"
		...
		<input type="text" name="memId" value="${user.memId}">
		<textarea name="msgContent" class="form-control" rows="3" title="留言"
