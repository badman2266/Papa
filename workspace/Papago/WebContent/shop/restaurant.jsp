				...
				<table class="table table-hover">
					<c:forEach var="bean" items="${select}" >
						<tr>
							<td>
								<table>
									<tr><td colspan="4"><strong><h4>${bean.name}</h4></strong></td></tr>
									<tr><th>類型：</th><td>${bean.shopType.shopType}</td><th>店家狀態：</th><td>${bean.shopStatusType.shopStatus}</td></tr>
									<tr><th>地址：</th><td colspan="3"><span id="address">${bean.shopAddress}</span></td></tr>
									<tr><th>電子信箱：</th><td>${bean.email}</td><th>電話：</th><td>${bean.phone}</td></tr>
									<c:if test="${not empty bean.web}">
										<tr><th>官方網站：</th><td colspan="3"><a href="${bean.web}" target="_blank">有</a></td></tr>
									</c:if>
									<tr><th>價格：</th><td>${bean.price}</td><th>價位區間：</th><td>${bean.priceType.priceType}</td></tr>
									<c:if test="${not empty bean.note}">
										<tr><td><blockquote>店家備註<br>${bean.note}<br></blockquote></td></tr>
									</c:if>
								</table>
							</td>
...
<%-- 			</c:if> --%>
			...
			<c:if test="${not empty msg}">
				...
				<c:forEach var="bean" items="${msg}">
					<li class="list-group-item">
						<a href='<c:url value="#" />'><img src='<c:url value="../images/icon_avatar.jpg" />' class="avatar" width="50"></a>
						<p class="name" >
			...
			<form action='<c:url value="/shop/papashop.controller" />' method="post">
				...
				<textarea name="msgContent" class="form-control" rows="3" title="留言" placeholder="留言..."></textarea>
				<br>
				...
				<div class="modal-body">
					<form action="<c:url value='/admin/shop/updateshop.controller'/>"
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<c:forEach var="bean" items="${select}" >
									<input type="text" value="${bean.shopId}" name="shopId" class="form-control">
								</c:forEach>
							</div>
						</div>	
						<div class="form-group">
